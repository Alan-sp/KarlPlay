...
private:
    ...
    QPushButton *sequentialButton;  // 顺序播放按钮
    QPushButton *loopButton;  // 循环播放按钮
    QPushButton *randomButton;  // 随机播放按钮

    QMediaPlaylist::PlaybackMode currentMode;

    void setupUI();  // 初始化UI组件
    void changePlayMode(QMediaPlaylist::PlaybackMode mode);  // 切换播放模式
    QPushButton *playModeButton;  // 选择播放模式的按钮
    QMenu *playModeMenu;  // 菜单
};
#endif // MAINWINDOW_H
